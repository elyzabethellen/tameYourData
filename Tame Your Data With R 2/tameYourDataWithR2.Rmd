...
##Transform
We'll bring in a second dataset here, transform it, and then transform our original dataset by doing a join on superhero names. For brevity's sake, this one is already tidied up.Let's take a look at it first.
...
second
```
I think we should do a transformation here, but let's do it after the join, so we transform all at once. 
...
###spread and gather
This data is a great candidate for transformation. Let's break up that status column into separate factors by using spread:
```{r spread}
#Use the spread command to spread the status column out. (Go to "wide.")
superInnerWide <- superInner %>% spread(status, measurement, convert=TRUE)
superInnerWide
```
...
How about our full-joined result?
```{r alignFull}
superFullSpread %>% ggplot(aes(sex)) +
  geom_bar(aes(fill=alignment)) 
```
Here you can see how NAs are handled in plotting. As a bonus exercise, handle the NAs in the full join and re-plot.
